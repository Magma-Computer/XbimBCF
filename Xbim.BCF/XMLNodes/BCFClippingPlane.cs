	...
	public class BCFClippingPlane
	{
		...
		public Vector Direction
		{
			...
			set
			{
				if (value.X == double.NaN || value.Y == double.NaN || value.Z == double.NaN)
				{
					throw new ArgumentException(GetType().Name + " - Direction - must contain X, Y and Z nodes containing a valid 64-bit floating-point value");
				}
		...
		public BCFClippingPlane(XElement node)
		{
			...
			Direction =
				new Vector(
					(double?)node.Element("Direction").Element("X") ?? double.NaN,
